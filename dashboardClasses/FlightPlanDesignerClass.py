...
airIP = "localhost" #192.168.208.6
groundIP = "localhost" #147.83.249.79
class VideoPlayer(tk.Toplevel):
    def __init__(self, parent, file_name):
        ...
        if path.exists(f"Videos/{file_name}"):
            ...
            time.sleep(1)
            response = requests.get(f"http://"+groundIP+":8105/media/videos/{file_name}")
...
class FlightPlanDesignerWindow:
    ...
    def runButtonClick(self):
        ...
        response_json = requests.get('http://'+groundIP+':8105/get_flight_plan/' + flightplan_id_ground).json()
        ...
        response = requests.get('http://'+airIP+':9000/get_flight_plan/' + response_json["Title"]).json()
        if response == None:
            data_flightplan = {
            ...
            headers = {"Content-Type": "application/json"}
            response_air = requests.post("http://"+airIP+":9000/add_flightplan", json=data_flightplan, headers=headers)
        ...
        response_air = requests.post('http://'+airIP+':9000/add_flight', json=data, headers=headers)
    ...
    def sendFlightPlanAir(self):
        ...
        if response == None:
            ...
            print("Sending data:", data_flightplan)
            headers = {"Content-Type": "application/json"}
            requests.post("http://"+airIP+":9000/add_flightplan", json=data_flightplan,
                                         headers=headers)
    ...
    def saveToDB(self, win, title):
        ...
        response = requests.post(
            "http://"+groundIP+":8105/add_flightplan", json = data, headers = headers
        )
    ...
    def populate_table(self):
        flights = requests.get("http://"+groundIP+":8105/get_all_flights").json()
    ...
    def loadFromDB(self, win):
        ...
        flightPlans = requests.get("http://"+groundIP+":8105/get_all_flightPlans").json()[
            "Waypoints"
        ]
    ...
    def display_image(self, image_name):
        ...
        if path.exists(f"Pictures/{image_name}"):
            ...
            response = requests.get(f"http://"+groundIP+":8105/media/pictures/{image_name}")
            if response.status_code == 200:
    ...
    def openImagesWindow(self):
        ...
        for i, image_name in enumerate(
            [item["namePicture"] for item in self.currentFlightData["Pictures"]]
        ):
